...
{   
    param(  [Switch]    $ReturnCollectionOnly,    
    ...
    {   # Command assumes $Model, $Vendor, and $MyPath already exist
        try     {   $Result = Get-RedfishChassis -erroraction stop
                    ...
                    $MyString1 = $MyString + 'PS:> Get-RedfishChassis' + $RCO + "`n"
                    $MyString2 = $MyString + 'PS:> Get-RedfishChassis' + $RCO + '| ConvertTo-JSON' + "`n"
                    $MyString1 = $MyString1 + ( $Result | Out-String )
                    $MyString2 = $MyString2 + ( $Result | ConvertTo-JSON | Out-String )
                    $MyFile1   = 'Examples\Chassis\Get-RedfishChassis.Example.' + $RCO  + $Vendor + '.txt'
                    $MyFile2   = 'Examples\Chassis\Get-RedfishChassis.Example.' + $RCO + $Vendor + '.json'
...
{   
    ...
    {   # Command assumes $Model, $Vendor, and $MyPath already exist
        try     {   $Result = Get-RedfishChassisPower -metricname $MetricName -erroraction stop
                    ...
                    $MyString2 = $MyString + 'PS:> Get-RedfishChassisPower -MetricName ' + $MetricName + " | ConvertTo-JSON`n"
                    $MyString1 = $MyString1 + ( $Result | Out-String )
                    $MyString2 = $MyString2 + ( $Result | ConvertTo-JSON | Out-String )
                    $MyFile1   = 'Examples\Chassis\Get-RedfishChassisPower-MetricName.' + $MetricName + '.Example.' + $Vendor + '.txt'
                    $MyFile2   = 'Examples\Chassis\Get-RedfishChassisPower-MetricName.' + $MetricName + '.Example.' + $Vendor + '.json'
                    $OutName1  = $MyPath.Replace('SNIASwordfish.psm1',$MyFile1)
                    $OutName2  = $MyPath.Replace('SNIASwordfish.psm1',$MyFile2)
                    write-verbose "The file to be written would be $OutName1 and $OutName2"
                    write-verbose "The Content to send to the file would be `n $MyString1 $MyString2"
...
{   
    ...
    {   # Command assumes $Model, $Vendor, and $MyPath already exist
        try     {   $Result = Get-RedfishChassisThermal -metricname $MetricName -erroraction stop
                    ...
                    $MyString1 = $MyString1 + ( $Result | Out-String )
                    $MyString2 = $MyString2 + ( $Result | ConvertTo-JSON | Out-String )
                    $MyFile1   = 'Examples\Chassis\Get-RedfishChassisThermal-MetricName.' + $MetricName + '.Example.' + $Vendor + '.txt'
                    $MyFile2   = 'Examples\Chassis\Get-RedfishChassisThermal-MetricName.' + $MetricName + '.Example.' + $Vendor + '.json'
                    $OutName1  = $MyPath.Replace('SNIASwordfish.psm1',$MyFile1)
                    $OutName2  = $MyPath.Replace('SNIASwordfish.psm1',$MyFile2)
                    write-verbose "The file to be written would be $OutName1 and $OutName2"
                    write-verbose "The Content to send to the file would be `n $MyString1 $MyString2"
