...
function Get-RedfishLabStorage
{   
    ...
    {   # Command assumes $Model, $Vendor, and $MyPath already exist
        try     {   if ( $ReturnCollectionOnly ) 
                        {   $Result = Get-RedfishStorage -returncollectiononly -erroraction stop
                            $RCO = '-ReturnCollectionOnly'
                        }
                        else 
                        {   $Result = Get-RedfishStorage -erroraction Stop 
                            $RCO = ''
                        }
                    $MyString  = '.EXAMPLE ' + "`n" + 'This example shows connecting to a ' + $Model + ' server.' + "`n"            
                    $MyString1 = $MyString + 'PS:> Get-RedfishStorage ' + $RCO + "`n"
                    $MyString2 = $MyString + 'PS:> Get-RedfishStorage ' + $RCO + ' | ConvertTo-JSON' + "`n"
                    $MyString1 = $MyString1 + ( $Result | Out-String )
                    $MyString2 = $MyString2 + ( $Result | ConvertTo-JSON | Out-String )
                    $MyFile1   = 'Examples\Storage\Get-RedfishStorage ' + $RCO + '.Example.' + $Vendor + '.txt'
                    $MyFile2   = 'Examples\Storage\Get-RedfishStorage ' + $RCO + '.Example.' + $Vendor + '.json'
